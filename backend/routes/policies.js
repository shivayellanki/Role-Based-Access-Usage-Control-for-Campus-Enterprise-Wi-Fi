  ...
  async (req, res) => {
    try {
      ...
      if (blocked_categories !== undefined) {
        ...
        if (Array.isArray(blocked_categories)) {
          parsedCategories = blocked_categories;
        } else if (typeof blocked_categories === 'string') {
          parsedCategories = blocked_categories.replace(/[{}]/g, '').split(',').map(s => s.trim()).filter(Boolean);
        }
        updateValues.push(parsedCategories);
      }
