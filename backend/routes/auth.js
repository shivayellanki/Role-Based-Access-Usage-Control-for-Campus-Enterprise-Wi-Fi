...
const createSession = async (userId, roleId, token, ipAddress, macAddress, deviceType, os, browser) => {
  ...
  const result = await pool.query(
    `INSERT INTO sessions (user_id, role_id, session_token, ip_address, mac_address, expires_at, device_type, os, browser)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     RETURNING id, started_at`,
    [userId, roleId, token, ipAddress, macAddress || null, expiresAt, deviceType || 'Unknown', os || 'Unknown', browser || 'Unknown']
  );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create session
      const session = await createSession(user.id, user.role_id, token, ipAddress, macAddress, deviceType, os, browser);
